...
def edgeDetect(images):
 ...
 for i in range(0,len(images)):
    ...
    prewittx = cv2.filter2D(images[i], -1, kernelx)
    prewitty = cv2.filter2D(images[i], -1, kernely)
    prewitt = prewittx + prewitty
    #--------------

    #**************
    #More the number of labels less connected components as the connected components are given the same label in the image
    sobelLabels, sN = label(sobel, neighbors=None, background=None, return_num=True, connectivity=None)
    result.append(sN)
    prewittLabels, pN = label(prewitt, neighbors=None, background=None, return_num=True, connectivity=None)
    result.append(pN)
...
def mnist_tutorial_cw(train_start=0, train_end=60000, test_start=0,
    ...
    cw_params = {'binary_search_steps': 1,
                 ...
                 'initial_const': 10}
    #edges = edgeDetect(X_test[:10])
    #images = X_test.reshape((X_test.shape[0], X_test.shape[1] * X_test.shape[2]))
    images = adv_inputs.reshape((adv_inputs.shape[0], adv_inputs.shape[1] * adv_inputs.shape[2]))
    #laplacian = cv2.Laplacian(images[0],cv2.CV_64F)
    edges = edgeDetect(adv_inputs)
    print('Length of edges: ')
    print(edges[:20])
    adv = cw.generate_np(adv_inputs,
                         **cw_params)
    #images = adv.reshape((adv.shape[0], adv.shape[1] * adv.shape[2]))
